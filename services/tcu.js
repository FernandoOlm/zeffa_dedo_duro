...
const org = apis.transparencia.orgaos.tcu;
...
export async function zeffaGastosTCU_unique() {
  return await reqT(
    apis.transparencia.endpoints.despesas_orgao + `?orgao=${org}`
  );
}

export async function zeffaAuditoriasTCU_unique() {
  return await reqT(apis.transparencia.endpoints.auditorias);
}